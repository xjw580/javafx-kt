...
abstract class PaneBaseBuilder<T : Pane> : LayoutBuilder<T>() {
    ...
    fun label(text: String, config: (Label.() -> Unit)? = null) {
        add(Label(text).apply { config?.invoke(this) })
    }

    fun label(config: (LabelBuilder.() -> Unit)? = null) {
        add(LabelBuilder().apply { config?.invoke(this) }.build())
    }

    fun title(config: (Title.() -> Unit)? = null) {
        val title = Title()
        config?.invoke(title)
        add(title)
    }

    fun title(text: String, config: (Title.() -> Unit)? = null) {
    ...
    fun <T> comboBox(item: List<T>, config: (ComboBox<T>.() -> Unit)? = null) {
        add(ComboBox<T>(FXCollections.observableArrayList(item)).apply { config?.invoke(this) })
    }

    fun <T> comboBox(config: (ComboBoxBuilder<T>.() -> Unit)? = null) {
        add(
    ...
    fun <T> filterComboBox(item: List<T>, config: (ComboBox<T>.() -> Unit)? = null) {
        add(FilterComboBox<T>(FXCollections.observableArrayList(item)).apply { config?.invoke(this) })
    }

    fun <T> filterComboBox(config: (FilterComboBoxBuilder<T>.() -> Unit)? = null) {
        add(
    ...
    fun button(text: String, config: (Button.() -> Unit)? = null) {
        add(Button(text).apply { config?.invoke(this) })
    }

    fun button(config: (ButtonBuilder.() -> Unit)? = null) {
        add(
            ButtonBuilder().apply {
                config?.invoke(this)
            }.build()
    ...
    fun textField(config: (TextFieldBuilder.() -> Unit)? = null) {
        add(
...
class GridPaneBuilder : PaneBaseBuilder<GridPane>() {
    ...
    class CellBuilder(
        ...
        fun label(text: String, config: (Label.() -> Unit)? = null) {
            node = Label(text).apply { config?.invoke(this) }
        }

        fun button(text: String, config: (Button.() -> Unit)? = null) {
            node = Button(text).apply { config?.invoke(this) }
        }

        fun textField(config: (TextField.() -> Unit)? = null) {
            node = TextField().apply { config?.invoke(this) ?: Unit }
        }
